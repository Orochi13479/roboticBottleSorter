...
% In the following example, we are rotating the first (shoulder_pan) and last (wrist_3) joints by pi/8 in 5 seconds.
[client, thirdGoal] = rosactionclient('/scaled_pos_joint_traj_controller/follow_joint_trajectory');
thirdGoal.Trajectory.JointNames = jointNames;
thirdGoal.Trajectory.Header.Seq = 1;
thirdGoal.Trajectory.Header.Stamp = rostime('Now', 'system');
thirdGoal.GoalTimeTolerance = rosduration(0.05);
bufferSeconds = 1; % This allows for the time taken to send the message. If the network is fast, this could be reduced.
durationSeconds = 5; % This is how many seconds the movement will take
...
endJointSend = rosmessage('trajectory_msgs/JointTrajectoryPoint');
nextJointState_123456 = [2.0944,-1.2462,0.5986,-1.85,-1.5708,0];
rad2deg(nextJointState_123456)
endJointSend.Positions = nextJointState_123456;
endJointSend.TimeFromStart = rosduration(durationSeconds);

thirdGoal.Trajectory.Points = [startJointSend; endJointSend];
pause(2);

thirdGoal.Trajectory.Header.Stamp = jointStateSubscriber.LatestMessage.Header.Stamp + rosduration(bufferSeconds);

% Then you can send this to the robot
sendGoal(client, thirdGoal);
...
% In the following example, we are rotating the first (shoulder_pan) and last (wrist_3) joints by pi/8 in 5 seconds.
[client, fourthGoal] = rosactionclient('/scaled_pos_joint_traj_controller/follow_joint_trajectory');
fourthGoal.Trajectory.JointNames = jointNames;
fourthGoal.Trajectory.Header.Seq = 1;
fourthGoal.Trajectory.Header.Stamp = rostime('Now', 'system');
fourthGoal.GoalTimeTolerance = rosduration(0.05);
bufferSeconds = 1; % This allows for the time taken to send the message. If the network is fast, this could be reduced.
durationSeconds = 5; % This is how many seconds the movement will take
...
endJointSend = rosmessage('trajectory_msgs/JointTrajectoryPoint');
nextJointState_123456 = [2.0944,-0.2985,0.5986,-1.85,-1.5708,0];
rad2deg(nextJointState_123456)
endJointSend.Positions = nextJointState_123456;
endJointSend.TimeFromStart = rosduration(durationSeconds);

fourthGoal.Trajectory.Points = [startJointSend; endJointSend];
pause(2);

fourthGoal.Trajectory.Header.Stamp = jointStateSubscriber.LatestMessage.Header.Stamp + rosduration(bufferSeconds);

% Then you can send this to the robot
sendGoal(client, fourthGoal);
